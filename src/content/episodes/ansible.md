...
imports the OpenTelemetry Python SDK, which Ansible does not ship, so install that on the
controller first. Then turn the plugin on in the `ansible.cfg` next to your playbook. Every
play becomes one trace, with a span per task per host that carries the host name, the module
and the result. The endpoint and the auth header are the standard OpenTelemetry variables in
the environment the play runs in, such as a CI job or the bastion's `.envrc`; the plugin has
no config-file key for them. Needs `community.general` 9.0 or newer for the protocol line;
older versions read `OTEL_EXPORTER_OTLP_TRACES_PROTOCOL=http/protobuf` from the environment
instead.
